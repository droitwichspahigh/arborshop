...
class AjaxTableEditor
{
	...
	public $storageType;
	public $debug = false;
	public $encryptStorageData = false;
	protected $encKey = 'c4j9803f90riepal';
	...
	public function setDefaults()
	{
		...
		$this->displayNum = isset($this->sessionData['displayNum']) ? $this->sessionData['displayNum'] : 20;
		...
		$this->storageType = isset($this->storageType) ? $this->storageType : 'web';
		$this->encryptStorageData = isset($this->encryptStorageData) ? $this->encryptStorageData : false;
	}
	...
	{
		if(!empty($value) && $this->encryptStorageData)
		{
	...
	public function handleFilterSearch()
	{
		...
		{
			$this->addJavascript('$("#' . $this->info['currentFilterId'] . '").select();',2);
		}
	}
	...
	public function resetColumnOrder()
	{
		...
		if(isset($storageData['order']))
		{
			$storageData['order'] = array();
		}
		$this->writeStorageData($storageData);
		$this->addJavascript($this->instanceName.'.toAjaxTableEditor(\'update_html\',\'\',{updateHistory: true});',1);
	}
	...
	public function setHiddenColumns()
	{
		...
		{
			$storageData = $this->getStorageData();
			$hiddenArr = isset($storageData['hidden']) ? $storageData['hidden'] : array();
	...
	public function updateHiddenColumns()
	{
		...
		$storageData['hidden'] = $hiddenArr;
		$this->writeStorageData($storageData);
		$this->addJavascript($this->instanceName.'.toAjaxTableEditor(\'update_html\',\'\',{updateHistory: true});',1);
	}
	
	public function writeStorageData($storageData)
	{
		$storageData = $this->encryptData($this->jsonEncode($storageData));
		if($this->storageType == 'cookie')
		{
			if(!setcookie($this->varPrefix,$storageData,time() + 60*60*24*7*365*2))
			{
				$this->warnings[] = 'To perform this function your browser must accept cookies.';
			}
		}
		else if($this->storageType == 'web')
		{
			$this->sessionData['storageData'] = $storageData;
		}
	}
	...
	public function getStorageData()
	{
		...
		if($this->storageType == 'cookie')
		{
			...
			{
				$storageData = $_COOKIE[$this->varPrefix];
			}
		}
		else if($this->storageType == 'web')
		{
			if(isset($this->sessionData['storageData'])) 
			{
				$storageData = $this->sessionData['storageData'];
			}
		}
		return $this->jsonDecode($this->decryptData($storageData),true);
	}
	...
	public function setColumnOrder()
	{
		...
		{
			$storageData = $this->getStorageData();
			$orderArr = isset($storageData['order']) ? $storageData['order'] : array();
	...
	public function getInputHtmlInfo($info,$row,$col,$jsFun,$addOrEdit,$editMult=false,$mateRowNum='')
	{
		...
		{
			$inputHtml .= call_user_func($info['format_input_fun'], $col, $defVal, $row, $inputInfo, $this->instanceName, $mateRowNum, $addOrEdit);
		}
		...
		else if(isset($info['join']) && is_array($info['join']) && !isset($info['join']['no_auto_select']))
		{
			...
			if (!empty($info['join']['alias'])) {
				$joinTable .= ' AS ' . $info['join']['alias'];
			}
			...
			foreach($this->tableColumns as $col => $info)
			{
				...
				if(isset($info['join']) && is_array($info['join']))
				{
					...
					else
					{
						...
						{
							// Changed to make alias the same as the table name (if no alias was given) 2015-06-22.
							$this->tableColumns[$col]['join']['alias'] = $info['join']['table'];
	...
	public function getDispNumDropDown()
	{
		$counter = 0;
		$value = 0;
		// $value = $this->displayNumInc; // commented because starting display num was off by inc
